...
// ---------------------------------------------------------------------------
// Single package rollback — FIXED: usuwa stare wrappery przed tworzeniem nowych
// ---------------------------------------------------------------------------
...
Run {} to diagnose.",
    ...
    // ── Zbierz nazwy binariów z OBU wersji ──────────────────────────────────
    let current_bins = collect_bin_names_from_dir(&curr_dir);
    let prev_bins    = collect_bin_names_from_dir(&prev_dir);
    ...
    // Usuń też wrappery z alternatywnymi nazwami (np. pkg-bin zamiast bin)
    // — skanuj /usr/bin w poszukiwaniu wrapperów dla tego pakietu
    ...
    if let Ok(manifest) = crate::manifest::Manifest::load_from_path(prev_dir.to_str().unwrap()) {
        for bin_name in &manifest.bins {
            let bin_rel = if let Some(explicit) = manifest.bin_paths.get(bin_name) {
                let p = prev_dir.join(explicit);
                if p.exists() { Some(explicit.clone()) } else { None }
            } else {
                crate::commands::install::find_binary_in_dir(&prev_dir, bin_name)
            };
...
/// Zbierz nazwy binariów z manifestu w danym katalogu (nie panikuj jeśli brak).
fn collect_bin_names_from_dir(dir: &Path) -> HashSet<String> {
    if !dir.exists() { return HashSet::new(); }
    match crate::manifest::Manifest::load_from_path(dir.to_str().unwrap_or("")) {
        Ok(m)  => m.bins.into_iter().collect(),
        Err(_) => HashSet::new(),
    }
}
...
fn remove_all_wrappers_for_package(pkg_name: &str) -> Result<()> {
    ...
    for entry in rd.flatten() {
        ...
        if let Ok(content) = fs::read_to_string(&path) {
            // Format wrappera: exec <hpm_path> run <pkg_name> <bin_rel>
            if content.contains("hpm run ") {
                for line in content.lines() {
                    if let Some(rest) = line.strip_prefix("exec ") {
                        let parts: Vec<&str> = rest.split_whitespace().collect();
                        if parts.len() >= 3 && parts[1] == "run" && parts[2] == pkg_name {
