...
pub fn upgrade() -> Result<()> {
    ...
    let local_version = if fs::metadata(LOCAL_VERSION_FILE).is_ok() {
        let data: serde_json::Value = serde_json::from_str(
            &fs::read_to_string(LOCAL_VERSION_FILE).into_diagnostic()?
        ).into_diagnostic()?;
    ...
    if compare_versions(&remote_version, &local_version) == std::cmp::Ordering::Greater {
        println!("{} Upgrading HPM from {} to {}...",
                 "→".yellow(), local_version.cyan(), remote_version.cyan());
        ...
        println!("{} HPM is already up to date ({})", "✔".green(), local_version.cyan());
    }
    Ok(())
}
