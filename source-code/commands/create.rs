...
pub fn create(name: Option<String>) -> Result<()> {
    ...
    // ── Tagi grupowe ─────────────────────────────────────────────────────────
    let tags_input = prompt("Group tags (comma-separated, e.g. development,cli,tools)", Some(""))?;
  ...
  <text x="32" y="42" font-size="28" text-anchor="middle" fill="white" font-family="sans-serif">{}</text>
    ...
    let info_hk = generate_info_hk(
        ...
        &bin_name, is_gui, needs_network, &categories, &build_type, &tags,
    );
...
fn generate_info_hk(
    ...
    // Tagi grupowe — lista w formacie HackerKit
    if !tags.is_empty() {
        out.push_str("-> tags => [\n");
        for tag in tags {
            out.push_str(&format!("    \"{}\"\n", tag));
    ...
    // FIXED: poprawna składnia HackerKit dla sandbox
    // filesystem to lista, nie mapa — użyj [] gdy pusta
    out.push_str("\n[sandbox]\n");
    ...
    // Pusta lista filesystem — poprawna składnia HackerKit
