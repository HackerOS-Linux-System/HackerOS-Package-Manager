...
pub struct Manifest {
    ...
    /// Automatycznie wykrywane przez load_from_path — true jeśli istnieje hooks/.
...
/// Obsługuje format mapowy { item => "" } (hk_parser 0.3.x).
/// Pusta mapa {} lub pusty string = pusta lista.
/// Gdy hk_parser doda obsługę [] — ten fallback nadal zadziała.
...
// ---------------------------------------------------------------------------
// Arch validation — FIXED: import Colorize dodany wyżej
// ---------------------------------------------------------------------------
...
pub fn check_arch_compatibility(declared: &str) -> Result<()> {
    ...
    if !matches {
        // FIXED: Colorize jest teraz w scope
        return Err(miette!(
...
impl Manifest {
    pub fn load_from_path(path: &str) -> Result<Self> {
        ...
        if let Some(bm) = bins_map {
            for (k, v) in bm {
                bins.push(k.clone());
                if let Ok(pv) = v.as_string() {
                    if !pv.is_empty() { bin_paths.insert(k.clone(), pv); }
                }
            }
        }
        ...
        if let Some(c) = conflicts_sec {
            for (k, v) in c { if is_empty_value(v) { conflicts.push(k.clone()); } }
        }
        ...
        let runtime_deb_deps = runtime_sec.map(|r| get_string_list(r, "deb_deps")).unwrap_or_default();
